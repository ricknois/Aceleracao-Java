...
Neste desafio já temos uma classe `FileReaderUtils`. Dentro dela existe o método `getLines()`, que faz a abertura e leitura de um arquivo que não existe. Atualmente esse método está lançando a exceção `FileNotFoundException`. Seu desafio é alterar esse método e, em vez de ele lançar a exceção, tratar esse erro e retornar um valor do tipo `String` contendo "Arquivo não existe". Simbora?
